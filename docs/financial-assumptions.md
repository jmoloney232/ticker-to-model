...
| Field | Default | Notes |
|---|---|---|
| `terminal_growth` | `max(1.5%, min(2.5%, 10Y yield))` | Nominal-GDP anchor with a floor. The unfloored `g ≤ rf` rule breaks in distorted-rate regimes — in 2020 it forced g ≈ 0.7% and cut every valuation 20–30% for non-business reasons. Overrides above `min(2.5%, 10Y)` warn (P5); `g ≥ WACC` is hard-blocked |
