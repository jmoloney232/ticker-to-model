...
| Valuation | WACC build-up (nested-IF synthetic-spread lookup generated from the engine's `SPREAD_TABLE` — parity by construction; live `kd_synthetic` toggle), stub/exponent cells from `valuation_date` − `fy0_end`, UFCF schedule referencing Model, then **one formula block per registry method** (Gordon TV, exit TV, EPV — each with semantic guards; the writer keeps a `BLOCK_BUILDERS` map keyed by method id and fails loudly for an id without a block), implied cross-checks, EV→equity bridge with **one column per method** (labels/order from the registry), per-share row per method, **value-of-growth rows** (Gordon − EPV per share, % of DCF, and a labeled reading for the value-destructive inversion) | **Formulas only** |
